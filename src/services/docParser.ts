...
function serialise(nodes: NodeListOf<ChildNode> | readonly Node[]): string {
...
function buildSubsection(h2: Element, nextStop: Element | null): Subsection {
...
function buildSection(h1: Element, nextH1: Element | null): Section {
...
export function parseGoogleDoc(html: string): DocData {
  ...
  const h1s = Array.from(parsed.querySelectorAll('h1'));

  const sections = h1s
    .map((h1, idx) => {
      const nextH1 = idx < h1s.length - 1 ? h1s[idx + 1] : null;
      return buildSection(h1, nextH1 ?? null);
    })
